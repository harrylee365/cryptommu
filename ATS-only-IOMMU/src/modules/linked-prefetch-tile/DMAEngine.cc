                       ...
                       RubyRequestType direction, CallbackBase* cb)
{
  if (!IsRedirectingToMemory()) {
    ...
    MemCmd cmd;
    //std::cout << "Timing Request in DMA Engine pAddr value" <<pAddr <<std::endl;
   ...
   //ML_LOG("DMAEngine", "memory request issued 0x" << std::hex << lAddr);
    ...
    assert(memObject);
    assert(direction == RubyRequestType_LD || direction == RubyRequestType_ST);
    //std::cout<<"Are you not using functional mode" << std::endl;
    if (direction == RubyRequestType_LD) {
...
{
  //std::cout<<"Finishing translation" << std::endl;
...
{
  uint8_t* data = new uint8_t [size];
  //std::cout<<"Reading from pMemAddr"<< pMemAddr<< "lMemAddr"<< lMemAddr<< std::endl;
