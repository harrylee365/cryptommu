...
public:
  ...
  bool free;
  uint64_t mruTick;
  IotlbEntry() : vpBase(0), ppBase(0), free(true), device_id(-1), mruTick(0) {}
  void setMRU()
  {
...
public:
  IOTLB(int _numEntries, int associativity) :
    numEntries(_numEntries), assoc(associativity)
  {
    ...
    entries = new IotlbEntry*[sets];

    for (int i = 0; i < sets; i++) {
      entries[i] = new IotlbEntry[assoc];
    }
  }
  virtual ~IOTLB()
  {
  ...
  bool lookup(uint64_t vp_base,  uint64_t device_id, uint64_t& pp_base, bool set_mru = true);
  void insert(uint64_t vp_base, uint64_t device_id, uint64_t pp_base);
  void flushAll();
};
...
private:
  ...
  std::map<uint64_t, std::list<const int32_t* > > MSHRs;
  std::list<const int32_t*> MISS;
...
public:
  ...
  /* IOTLB 64-entry Fully Associative */
  IOTLB* iotlb = new IOTLB(64, 64); //IOTLB(entries, assoc)
  int iommu_hits=0;
  int iommu_access=0;
  int iommu_misses=0;
  uint64_t mshr_entries =0;
  ...
  void setupWalk(int thread, uint64_t vAddr, uint64_t device_id);
