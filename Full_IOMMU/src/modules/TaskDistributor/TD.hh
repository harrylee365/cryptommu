...
class TD
{
  ...
  void AllocateMore();
  void translate(int src, uint64_t logicalPage, uint64_t node_id);
  ...
  typedef MemberCallback3<TD, int, uint64_t, uint64_t, &TD::translate> translateCB;
...
public:
  ...
  uint64_t getTlbHits()
  {
    printf("TD TLB hits %d TLB mshrhits %d ",hits,mshrhits);
    return hits + mshrhits;
  }
  uint64_t getTlbMisses()
  {
    printf("TD TLB misses %d",misses);
    return misses;
  }
