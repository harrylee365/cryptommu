                             ...
                             Arg1CallbackBase<uint64_t>* accessViolation)
{
  ...
  tlbMemory = new TLBMemory(1, 0); // fully associative
...
/*These are not part of CryptoMMU checks */
...
TLBMemory::lookup(uint64_t vp_base, uint64_t& pp_base, bool set_mru)
{
  ...
  printf("TD way %d ways %d sets %d",way, ways,sets);
...
TLBMemory::insert(uint64_t vp_base, uint64_t pp_base)
{
  ...
  printf("TD TLB sets %d and ways %d",sets,way);
  for (int i = 0; i < sets; i++) {
