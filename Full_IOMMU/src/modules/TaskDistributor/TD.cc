...
TD::MsgHandler(int src, const void* packet, unsigned int packetSize)
{
  ...
  case (LCACC_CMD_TLB_MISS): { //tlb miss from lca
    ...
    TaskDistributor::SimicsInterface::RegisterCallback(
      translateCB::Create(this, src, logicalPage, node_id), hitLatency);
  }
  break;
...
/*CRyptoMMU: This function just takes the address and supply it to IOMMU*/
void
TD::translate(int src, uint64_t vp_base, uint64_t node_id)
{
    uint64_t pp_base;
    ...
    unsigned int userProcess = cfuUseMap[src];
      ...
      outMsg[4] = bc.u32[0];
      outMsg[5] = bc.u32[1];
      netPort->SendMessage(core, outMsg, sizeof(outMsg),20); /*IOTLB access latency*/
...
TD::TD()
{
  ...
  //printf("TD TLB numEntries: %d associativity: %d hitlatency: %d", numEntries,associativity,hitLatency);
  if (numEntries > 0) {
    tlb = new TLBMemory(numEntries, associativity);
    //printf("I am hitting in fixed size TD TLB");
  } else {
    //printf("I am hitting in infinite size TLB memory");
