...
void 
DMAController::finishVerification (uint64_t vp_base)
{
  assert(MAC_queue.find(vp_base)!= MAC_queue.end());
   std::list<TransferData*> &tds = MAC_queue[vp_base];
  std::list<TransferData*>::iterator it;

  for (it = tds.begin(); it != tds.end(); it++) {
    TransferData* td = (*it);
    //std::cout << "dmaDevice" << dmaDevice << std::endl; 
    // set the physical address to complete the translation
     dmaInterface->finishTranslation(dmaDevice, td);
     MAC_queue.erase(vp_base);
  }
...
DMAController::translateTiming(TransferData* td)
{
  ...
  uint64_t MAC_dma;
  /*
  vaddr 6325376 offset 1152 vpbase 6324224
  */
